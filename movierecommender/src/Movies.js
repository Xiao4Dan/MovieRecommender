...
export default class Movies extends Component {
  constructor(props) {
    ...
    this.fetch_posters = this.fetch_posters.bind(this);
    console.log(data);
  }
  ...
  get_movies(searchText) {
    let master = this;
    axios
      .get("https://www.omdbapi.com?apikey=3fe96115&s=" + searchText)
      .then(function (res) {
  ...
  rate_movie(e) {
    ...
    temp_data[temp_user][e.target.parentElement.id] = parseInt(
      e.target.innerHTML
    );
    console.log(
    ...
    for (var newUser in data) {
      ...
      if (newUser !== currentUser && commonMovies.length !== 0) {
        ...
        for (var i = 0; i < commonMovies.length; i++) {
          simTop +=
            (data[newUser][commonMovies[i]] - newAvg) *
            (data[currentUser][commonMovies[i]] - currentAvg);
          currentSum += Math.pow(
            data[currentUser][commonMovies[i]] - currentAvg,
            2
          );
  ...
  async fetch_posters(data) {
    const target = document.querySelector("#posters");
    target.innerHTML = '';
    console.log(data.recommended);
    if (data.recommended.movies) {
        data.recommended.movies.splice(0,5).map(async (movie) => {
          try {
            await axios
              .get("https://www.omdbapi.com?apikey=3fe96115&i=" + movie[0])
              .then((res) => {
              target.innerHTML += `<div><img src=${res.data.Poster} alt=${res.data.Title}/></div>`;
              });
          } catch (err) {
            console.log(err);
          }
    ...
    return (
      <div className="App-body">
        <div className="App-search">
          ...
          <h2 id="prompt">
            The ultimate movie recommender and similarity calculator.
          </h2>
          <input
            id="searchbar"
          ...
          ></input>
          <button id="btn" onClick={this.handle_toggle}>Calculate between friends</button>
        </div>
        <div className="App-movieContainer">{movie_search}</div>
        <div className="toggle" id="toggle" onClick={this.handle_toggle}>
          &equiv;
        </div>
        <div className="App-dashboard" id="App-dashboard">
          ...
          <div id="chart">
            <h2>Similarities</h2>
            <Line
              data={this.state.recommendation.chart}
              options={{
          ...
          <div id="top-friends">
            <Friends data={this.state.recommendation} />
          </div>
          <div id="recommendations">
            <RecMovies
              data={this.state.recommendation}
              asyncFn={this.fetch_posters}
            />
          </div>
        </div>
      </div>
...
const RecMovies = (props) => {
  return (
    <>
      <h2 onClick={() => props.asyncFn(props.data)}>Recommend</h2>
      <div id="posters"></div>
    </>
  );
};
...
const Friends = (props) => {
  ...
  return (
    <ul id="friends">
      {props.data.recommended.friends &&
        props.data.recommended.friends.map((f, i) => {
          return (
            <li key={f[0]}>
              <span id="order">{i + 1}</span>
              <span id="name">{f[0]}</span>
              <span id="similarity">{f[1] * 100}%</span>
            </li>
          );
        })}
    </ul>
